...
import optparse, os, sys, base64
import jwt ## python3 -m pip install jwt

try:
    from dpapick3 import blob, masterkey, registry
except ImportError:
    raise ImportError('Missing dpapick3, please install via pip3 install dpapick3.')
...
if __name__ == '__main__':
    ...
    parser.add_option('--system', metavar='HIVE', dest='system', default=os.path.join('Windows','System32','config','SYSTEM'), help='SYSTEM hive')
    parser.add_option('--security', metavar='HIVE', dest='security', default=os.path.join('Windows','System32','config','SECURITY'), help='SECURITY hive')
