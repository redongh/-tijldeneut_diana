...
def check_parameters(options, args):
    ...
    if not args or len(args) != 1: sys.exit('You must provide a system vaults directory.')
    elif not os.path.isdir(args[0]): sys.exit('You must provide a system vaults directory.')
    if not options.masterkeydir: sys.exit('Cannot decrypt anything without master keys.')
    for x in [options.system, options.security]: 
        if not os.path.exists(x): exit('Error finding file "{}"'.format(x))
    if not os.path.exists(options.software): options.software = None
    #if not options.system or not options.security: sys.exit('You must provide SYSTEM and SECURITY hives.')
...
def main(sVaultFolder, sMasterkey, sSystem, sSecurity, sSoftware = None, sTargetSID = None, boolOutput = True):
            ...
            with open(filepath, 'rb') as vcrdfile:
                ...
                if bKeyAES256 and bData:
                    ...
                    if dicContainers[1].startswith('NGC'.encode('UTF-16LE')): ## Should be "NGC Local Accoount Logon Vault Resource" in utf-16le
                        ...
                        if sTargetSID and not sTargetSID == sSID: continue
                        if boolOutput:
...
if __name__ == '__main__':
    ...
    parser.add_option('--masterkey', metavar='DIRECTORY', dest='masterkeydir', default=os.path.join('Windows','System32','Microsoft','Protect','S-1-5-18','User'))
    ...
    parser.add_option('--software', metavar='HIVE', dest='software', help='Optional, for username retrieval', default=os.path.join('Windows','System32','config','SOFTWARE'))
